...
def merge_dataset(dataset_path, workers_num):
    ...
    for i in range(workers_num):
        tmp_dataset_reader = open("dataset/temp/dataset-tmp-" + str(i) + ".pt", "rb")
        while True:
        ...
        os.remove("dataset/temp/dataset-tmp-" + str(i) + ".pt")
...
class BertFlowDataset(Dataset):
    ...
    def worker(self, proc_id, start, end):
        ...
        document = []
        pos = 0
        dataset_writer = open("dataset/temp/dataset-tmp-" + str(proc_id) + ".pt", "wb")
        with open(self.corpus_path, mode="r", encoding="utf-8") as f:
            try:
